...
// ═══════════════════════════════════════════════════
// REALISTIC KEYWORD GENERATOR — مبني على منتجاتك الحقيقية فقط
// ═══════════════════════════════════════════════════
function generateKeywordSuggestions(baseKeyword, minWords = 2, maxWords = 4) {
    if (!baseKeyword || !baseKeyword.trim()) return { matched: 0, results: [] };

    const kwTokens = cleanTokens(baseKeyword);
    if (!kwTokens.length) return { matched: 0, results: [] };
    const kwSet = new Set(kwTokens);

    // فلترة المنتجات التي تحتوي فعلياً كل كلمات الإدخال (بعد التطبيع)
    const matching = allProducts.filter(p => {
        const titleTokens = new Set(cleanTokens(p.designTitle));
        return [...kwSet].every(t => titleTokens.has(t));
    });

    if (!matching.length) return { matched: 0, results: [] };

    const freq = new Map(), bsrMap = new Map(), exampleMap = new Map();
    ...
    matching.forEach(p => {
        for (let n = minWords; n <= maxWords; n++) {
            ...
            const seen = new Set();
            phrases.forEach(ph => {
                const phTokens = ph.split(' ');
                const containsKeyword = [...kwSet].every(t => phTokens.includes(t));
                if (!containsKeyword || seen.has(ph)) return;
                seen.add(ph);
                freq.set(ph, (freq.get(ph) || 0) + 1);
                if (!bsrMap.has(ph)) bsrMap.set(ph, []);
                bsrMap.get(ph).push(p.bsrNumber);
                if (!exampleMap.has(ph)) exampleMap.set(ph, p.designTitle);
            });
        }
    });

    const results = [...freq.entries()].map(([phrase, count]) => {
        const b = bsrMap.get(phrase) || [];
        return {
            phrase,
            count,
            avgBSR: Math.round(b.reduce((a, x) => a + x, 0) / b.length),
            minBSR: Math.min(...b),
            exampleTitle: exampleMap.get(phrase)
        };
    }).sort((a, b) => b.count - a.count || a.avgBSR - b.avgBSR).slice(0, 40);
...
function renderKeywordSuggestions() {
    ...
    const { matched, results } = generateKeywordSuggestions(keyword);

    if (matched === 0) {
        container.innerHTML = `<div class="niche-item" style="text-align:center;color:#94a3b8;">
            ⚠️ لم تُستخدم كلمة "${escHtmlSafe(keyword)}" في أي منتج عندك بعد — لا نتائج وهمية، جرّب كلمة أخرى.
        </div>`;
        return;
    }
    ...
    if (!results.length) {
        container.innerHTML = `<div class="niche-item" style="text-align:center;color:#94a3b8;">
            وُجدت ${matched} منتج يحتوي هذه الكلمة، لكن لا عبارات كافية الطول لاستخراجها.
        </div>`;
        return;
    }
    ...
    container.innerHTML = results.map((r, i) => {
        ...
        return `<div class="niche-item" data-action="search-keyword" data-keyword="${escHtmlSafe(r.phrase)}">
            <div class="niche-main-row">
                <div class="niche-left">
                    <span class="niche-rank" style="background:${rc}22;color:${rc};">#${i + 1}</span>
                    <span class="niche-keyword" title="${escHtmlJS(r.exampleTitle)}">${escHtmlSafe(r.phrase)}</span>
                </div>
                <div class="niche-stats">
                    <span class="niche-count" style="background:${rc};">${r.count} منتج حقيقي</span>
                    <span class="niche-bsr">avg:${r.avgBSR.toLocaleString()}</span>
                    <span class="niche-bsr">🏆${r.minBSR.toLocaleString()}</span>
                    <span class="niche-action">
                        <button class="amazon-search-btn" data-action="amazon-search" data-keyword="${escHtmlSafe(r.phrase)}">
                            <i class="fab fa-amazon"></i> Search
                        </button>
                    </span>
                </div>
            </div>
            <div class="niche-bar-bg"><div class="niche-bar-fill" style="width:${bw}%;background:${rc};"></div></div>
        </div>`;
    }).join('');
}
...
async function loadProducts() {
    try {
        ...
        // تحديث العداد بصمت (العنصر مخفي في HTML)
...
function setupEventDelegation() {
    ...
    if (nichesContainer) {
      const genContainer = document.getElementById('keywordGenResults');
if (genContainer) {
...
function renderProducts(products) {
    ...
    // إعادة تهيئة الـ infinite scroll
    currentFilteredForScroll = products;
    currentPage = 0;

    // مسح الـ grid وإنشاء جديد فارغ
